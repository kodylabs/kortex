...
```json
{
  "mcpServers": {
    "kortex": {
      "command": "bun",
      "args": ["run", "/path/to/kortex/src/index.ts"]
    }
  }
}
```
...
bun run src/index.ts rebuild   # re-index all vault files from scratch
bun run src/index.ts config    # show active configuration
```

## Config: `~/.kortex-mcp/config.json`

Created automatically on first run. Key fields:
- `vault_path` — default `~/kortex-kb`
- `ollama_url` — default `http://127.0.0.1:11434`
- `embedding_model` — default `nomic-embed-text`
- `fallback_search` — `"fts5"` (keyword) when ollama is offline
